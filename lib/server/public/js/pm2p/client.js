  ...
  return {
    ...
    enrichStats: function(systemStats){
      ...
      for (p in updatedStats.processes) {
        ...
        process.started = moment( process.pm2_env.created_at ).fromNow();
        process.mem_text = pm2d.bytesToSize( process.monit.memory, 2);
        process.unstable_restarts = process.pm2_env.restart_time;
        process.status_text = process.pm2_env.status === 'online' ? 'online' : 'error';
      }
